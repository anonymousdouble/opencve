...
def test_check_summary(create_cve, handle_events, open_file):
    ...
    assert event.details == {'changed': {'en': {'old': 'The Requests package before 2.20.0 for Python sends an HTTP Authorization header to an http URI upon receiving a same-hostname https-to-http redirect, which makes it easier for remote attackers to discover credentials by sniffing the network.', 'new': 'The summary has been changed.'}}, 'added': {}, 'removed': {}}
