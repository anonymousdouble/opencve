...
from constants import SQL_ADVISORY, SQL_EVENTS, SQL_CVE
...
class AdvisoriesHandler(DiffHandler):
    def handle(self):
        run_sql(
            query=SQL_ADVISORY,
            parameters={
                "extras": Json(self.right.pop("extras")),
                "related_cves": Json(self.right.pop("related_cves")),
                "path": self.path,
                "source": self.source,
                **self.right,
            },
        ...
        related_cves = self.right.get("related_cves")
        if not related_cves:
            return

        # Create a change for each related CVE
        for cve in related_cves:
            self.create_change(cve, {
                "type": "new_advisory",
                "details": {"source": self.source, "key": self.right.get("key")},
            })
